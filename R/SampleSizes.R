#Need to go through and check in dem and big for names
#File 1:
...
SampleSizes<-function(df1, df2, PSU1, strata1, size1, demo1, demoyesno, stratayesno, strata2,
		mkvalue, mkvar, ssyesno, ssvar, sstotal, ppstype, nminimum, 
		Parcost, PSUcost, PSUV, PSUmean, optyesno, costyesno)
	{
...
#calculate n for those with inc==1 first 
	...
	if (nrow(big)>0) {
		if (demoyesno=="0"){
			...
			big$ST2_ANT_SSU<-big$strata.n*(big[,size1]/big$strata.pop)/length(demo1)}
...
#Join dem/x and big and add in other n's
		if (demoyesno==0) {
			...
			x$demo.n.new<-z$n.new[x$strata.id]/length(demo1) #changed to n.new
...
#Back to if there are no PSU with inc=1
		...
		if (demoyesno==0){
			x<-dem
			x$demo.n<-z$ST1_ANT_SSU[x$strata.id]/length(levels(x$DOMAIN_ID))
...
#Add in extras and General Clean-up
	...
	sstotal<<-round(sum(z$ST1_ANT_SSU))
	if(costyesno==0) z$ST1_COST <-(z[,PSUcost]*z$ST1_SEL_PSU)+(z[,Parcost]*z$ST1_ANT_SSU)
	if (optyesno==0) z$STR_VARIANCE<-(z[,Vamong]/z$ST1_SEL_PSU)+(z[,Vwithin]/z$ST1_ANT_SSU)
	if (optyesno==0 & demoyesno==1) {
		TotalVar<<-sum((z$ww^2)*z$STR_VARIANCE)
		COEF<<-(sqrt(TotalVar)/T.Var)*100
		...
		Deff<<-TotalVar/((VarAmongStrat+VarWithinStrat)/sum(z$ST1_ANT_SSU))
	}
